...
This documents how to produce and install a distributable build of the GUI
front-end (`irondict-gui`). The CLI (`irondict-cli`) ships the same way minus the
desktop entry and icon.

## Release build

```sh
cargo build --release -p irondict-gui
```
...
and free of debug symbols. The binary lands at `target/release/irondict-gui`.
...
2. `<exe-dir>/../share/irondict/gcide` — relative to the installed binary, so any
   `--prefix` works (`/usr/bin/irondict-gui` → `/usr/share/irondict/gcide`).
...
|----------|--------|-------------------------------|
| Binary | `target/release/irondict-gui` | `/usr/bin/irondict-gui` |
| Desktop entry | `packaging/irondict.desktop` | `/usr/share/applications/irondict.desktop` |
| Icons (PNG) | `crates/gui/assets/icons/hicolor/<size>/apps/irondict.png` | `/usr/share/icons/hicolor/<size>/apps/irondict.png` |
| Scalable icon | `crates/gui/assets/icons/irondict.svg` | `/usr/share/icons/hicolor/scalable/apps/irondict.svg` |
| GCIDE data | `crates/core/assets/gcide/*` | `/usr/share/irondict/gcide/` |

The `.desktop` `Exec=irondict-gui` and `Icon=irondict` match the installed binary
name and the icon basename. `StartupWMClass=irondict` lets the compositor pair the
...
```sh
prefix=/usr
install -Dm755 target/release/irondict-gui    "$DESTDIR$prefix/bin/irondict-gui"
...
`crates/gui/assets/icons/irondict.svg` is the source of truth (an open book on the
indigo accent tile, matching the GUI fallback accent `#4F46E5`). The PNG raster
sizes are generated from it:

```sh
cd crates/gui/assets/icons
for s in 16 32 48 64 128 256 512; do
...
- **IBM Plex Sans** (bundled under `crates/gui/assets/fonts/`) is SIL OFL 1.1.
